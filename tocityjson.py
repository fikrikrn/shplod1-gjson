...
def obj_to_cityjson(input_folder, output_file, epsg):
    ...
    with tqdm(total=len(obj_files), desc="Processing OBJ files", unit="file") as pbar:
        for index, obj_file in enumerate(obj_files):
            ...
            pbar.update(1)  # Update progress bar setiap file selesai

    # Menyimpan hasil CityJSON
    save_cityjson(cityjson, output_file)
    print(f"File CityJSON berhasil disimpan pada {output_file}")
